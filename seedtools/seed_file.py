...
# Load persistent data path from JSON config
DATA_PATH = return_data_path()


# -------------------------------
# Check data file and seed file
# -------------------------------
...
# -------------------------------
# Check seed file status (mini)
# -------------------------------
def check_data_mini(filename):
    seed_file_name = get_Seed_name(filename)
    return seed_file_name in os.listdir(DATA_PATH)
...
def register_csv(data, filename, desc=None):
    if check_data_mini(filename):
        os.remove(connect(get_Seed_name(filename)))  # remove old seed file
        print("Found existing seed file, removed it.")
    ...
    data_seed["desc"] = desc
    data_seed["version_names"] = []  # new version names will be added here

    seed_name = write_seed(filename, data_seed)
    print(f"Seed `{seed_name}` Registered Successfully")
...
class load_seed:
    def __init__(self, filename, version=None, quiet=False):
        ...
        self.cols_status = {"drop_cols": False, "map_cols": False, "text_cols": False}
        self.cols = {"drop_cols": [], "map_cols": {}, "text_cols": []}

        # VERSION != NONE + SEED STATUS
        if version is not None and seed_status:
            if version in read_version_names(filename):
                ...
                for col in self.cols_status.keys():
                    if not self.cols_status[col]:
                        print_u(f"Column not found : {col}  Re-registering seed file")
                        self.register()
                        self.register_version(
                            version,
                            drop_cols=self.cols["drop_cols"],
                            cols_maps=self.cols["map_cols"],
                            text_cols=self.cols["text_cols"]
                        )
        ...
        # VERSION == NONE + SEED STATUS
        elif seed_status and version is None:
            self.data = pd.read_csv(connect(self.filename))
            display_Seed_file(filename, not quiet)
    ...
    def terminate_version(self, vname):
        if self.termination_status and vname is not None:
            ...
            print(f"Can't terminate version, version `{vname}` not found or you passed `None`")
